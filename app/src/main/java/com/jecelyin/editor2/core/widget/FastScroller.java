...
package com.jecelyin.editor2.core.widget;
...
import com.jecelyin.editor2.core.text.Selection;
...
class FastScroller {
    ...
    private final Runnable mDeferStartDrag = new Runnable() {
        public void run() {
//            if (mJecEditText.mIsAttached) {
//            if (mJecEditText.isAttachedToWindow()) {
            beginDrag();

            final int viewHeight = mJecEditText.getHeight();
            // Jitter
...
//            }
    ...
    public FastScroller(Context context, JecEditText textView) {
        mJecEditText = textView;
        init(context);
    }
...
//            position = mJecEditText.isLayoutRtl() ?
            position = mJecEditText.getLayoutDirection() == TextView.LAYOUT_DIRECTION_RTL ?
    ...
    public void setState(int state) {
        switch (state) {
            ...
            case STATE_DRAGGING:
                ...
                break;
            case STATE_EXIT:
                final int viewWidth = mJecEditText.getWidth();
                final int top = mThumbY;
                final int bottom = mThumbY + mThumbH;
                final int left;
                final int right;
                switch (mJecEditText.getLayoutDirection()) {
                ...
                mJecEditText.invalidate(left, top, right, bottom);
                break;
        }
        mState = state;
    ...
    private void init(Context context) {
        ...
        useThumbDrawable(context, context.getResources().getDrawable(R.drawable.scrollbar_handle_accelerated_anim2)); //ta.getDrawable(ta.getDrawable(THUMB_DRAWABLE))
...
//        mScrollCompleted = true;
        ...
        if (mJecEditText.getWidth() > 0 && mJecEditText.getHeight() > 0) {
            onSizeChanged(mJecEditText.getWidth(), mJecEditText.getHeight(), 0, 0);
        }
        ...
        setScrollbarPosition(mJecEditText.getVerticalScrollbarPosition());
    }
    ...
    public void draw(Canvas canvas) {
        ...
        final int scrollX = mJecEditText.getScrollX();
        ...
        if (mTrackDrawable != null) {
            ...
            mTrackDrawable.setBounds(trackLeft, halfThumbHeight,
                    trackLeft + trackWidth, mJecEditText.getHeight() - halfThumbHeight);
                ...
                switch (mPosition) {
                    ...
                    case View.SCROLLBAR_POSITION_LEFT:
                        left = Math.min(mThumbDrawable.getBounds().right + mThumbW,
                                mJecEditText.getWidth() - mOverlaySize);
                        break;
                }

                int top = Math.max(0,
                        Math.min(y + (mThumbH - mOverlaySize) / 2, mJecEditText.getHeight() - mOverlaySize));
    ...
    void onScroll(JecEditText view, int firstVisibleItem, int visibleItemCount,
                  int totalItemCount) {
...
//        mScrollCompleted = true;
    ...
    Object[] getSections() {
        if (mListAdapter == null && mJecEditText != null) {
            getSectionsFromIndexer();
        }
        return mSections;
    }
...
//        Adapter adapter = mJecEditText.getAdapter();
...
//        }
    ...
    void scrollTo(float position) {
        int count = mJecEditText.getLineCount();
...
//
////            if (mJecEditText instanceof ExpandableListView) {
////                ExpandableListView expList = (ExpandableListView) mJecEditText;
////                expList.setSelectionFromTop(expList.getFlatListPosition(
////                        ExpandableListView.getPackedPositionForGroup(index + mListOffset)), 0);
////            } else if (mJecEditText instanceof ListView) {
////                ((ListView) mJecEditText).setSelectionFromTop(index + mListOffset, 0);
////            } else {
//                mJecEditText.setSelection(index + mListOffset);
////            }
//        } else {
...
//                mJecEditText.setSelection(index + mListOffset);
//            }
        int offset = mJecEditText.getLayout().getLineStart(index);
        Selection.setSelection(mJecEditText.getText(), offset, offset);
        sectionIndex = -1;
//        }
...
//        if (mSectionIndexer == null || !mMatchDragPosition) {
        return ((mJecEditText.getHeight() - mThumbH) * firstVisibleItem)
...
//
//        final int trackHeight = mJecEditText.getHeight() - mThumbH;
//
...
//
//        final View child = mJecEditText.getChildAt(0);
//        final float incrementalPos = child == null ? 0 : firstVisibleItem +
//                (float) (mJecEditText.getPaddingTop() - child.getTop()) / child.getHeight();
...
//            final View lastChild = mJecEditText.getChildAt(visibleItemCount - 1);
//            final float lastItemVisible = (float) (mJecEditText.getHeight() - mJecEditText.getPaddingBottom()
...
//        return result;
    ...
    private void cancelFling() {
        ...
        mJecEditText.onTouchEvent(cancelFling);
        cancelFling.recycle();
    }

    void cancelPendingDrag() {
        mJecEditText.removeCallbacks(mDeferStartDrag);
        mPendingDrag = false;
    }

    void startPendingDrag() {
        mPendingDrag = true;
        mJecEditText.postDelayed(mDeferStartDrag, PENDING_DRAG_DELAY);
    }
    ...
    void beginDrag() {
        ...
        if (mJecEditText != null) {
            mJecEditText.requestDisallowInterceptTouchEvent(true);
//            mJecEditText.reportScrollStateChange(OnScrollListener.SCROLL_STATE_TOUCH_SCROLL);
        }
...
//                    if (!mJecEditText.isInScrollingContainer()) {
                    beginDrag();
                    return true;
//                    }
...
//                startPendingDrag();
            ...
            if (mState == STATE_DRAGGING) {
                if (mJecEditText != null) {
                    ...
                    mJecEditText.requestDisallowInterceptTouchEvent(false);
//                    mJecEditText.reportScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE);
                }
                setState(STATE_VISIBLE);
...
//                }
    ...
    boolean isPointInside(float x, float y) {
        ...
        switch (mPosition) {
            default:
            case View.SCROLLBAR_POSITION_RIGHT:
                inTrack = x > mJecEditText.getWidth() - mThumbW;
                break;
            case View.SCROLLBAR_POSITION_LEFT:
                inTrack = x < mThumbW;
                break;
